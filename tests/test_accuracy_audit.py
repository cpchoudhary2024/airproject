...
# --------------------------------------------------------------------------
# External data integrations: OpenAQ collocation, Open-Meteo pollution rose
# --------------------------------------------------------------------------
...
def _openaq_rows(n, start_hour):
    """n synthetic OpenAQ hourly rows with distinct, sequential UTC hour keys."""
    base = pd.Timestamp("2026-01-01", tz="UTC") + pd.Timedelta(hours=start_hour)
    out = []
    for i in range(n):
        ts = (base + pd.Timedelta(hours=i)).strftime("%Y-%m-%dT%H:%M:%S")
        out.append({"period": {"datetimeFrom": {"utc": ts}}, "value": 10.0 + (i % 5)})
    return out


def test_openaq_pagination_follows_multiple_pages(monkeypatch):
    """A single page caps at 1000 hours (~42 days). Without pagination, any longer
    analysis window -- any realistic multi-month deployment -- would silently return
    a truncated, non-representative slice of the reference record."""
    import httpx
        ...
        def get(self, url, params=None, **kw):
            if url.endswith("/locations"):
                return _FakeResp({"results": [{
                    "name": "Test Monitor", "distance": 500,
                    "provider": {"name": "TestAgency"},
                    "coordinates": {"latitude": 1.0, "longitude": 2.0},
                    "sensors": [{"id": 42, "parameter": {"id": 2, "name": "pm25"}}],
                }]})
            if url.endswith("/sensors/42/hours"):
                page = int((params or {}).get("page", 1))
                # page 1 full (1000) -> must continue; page 2 partial (500) -> must stop.
                if page == 1:
                    return _FakeResp({"results": _openaq_rows(1000, start_hour=0)})
                if page == 2:
                    return _FakeResp({"results": _openaq_rows(500, start_hour=1000)})
                return _FakeResp({"results": []})
            raise AssertionError(f"unexpected URL {url}")

    monkeypatch.setattr(external.httpx, "Client", _FakeClient)
    monkeypatch.setenv("OPENAQ_API_KEY", "test-key")

    result = external.fetch_openaq_reference(1.0, 2.0, "2026-01-01", "2026-03-01")
    assert len(result["series"]) == 1500, "expected both pages merged (1000 + 500)"
    assert result["truncated"] is False


def test_openaq_pagination_flags_truncation_at_safety_cap(monkeypatch):
    import httpx
    from app import external
    ...
    class _FakeClient:
        ...
        def get(self, url, params=None, **kw):
            ...
            if url.endswith("/sensors/42/hours"):
                page = int((params or {}).get("page", 1))
                # Every page is full: pagination never naturally terminates.
                return _FakeResp({"results": _openaq_rows(1000, start_hour=(page - 1) * 1000)})
            raise AssertionError(f"unexpected URL {url}")

    monkeypatch.setattr(external.httpx, "Client", _FakeClient)
    monkeypatch.setenv("OPENAQ_API_KEY", "test-key")

    result = external.fetch_openaq_reference(1.0, 2.0, "2026-01-01", "2028-01-01")
    assert result["truncated"] is True


def test_pollution_rose_dominant_sector_uses_mean_not_total():
    """The prevailing wind direction accumulates the largest TOTAL PM2.5-hours almost
    by construction, regardless of whether pollution is actually elevated from that
    direction. A source-attribution reading needs the sector with elevated MEAN
    concentration, not the sector the wind simply blew from most often."""
    labels = ["N", "NNE", "NE", "ENE", "E", "ESE", "SE", "SSE",
              "S", "SSW", "SW", "WSW", "W", "WNW", "NW", "NNW"]
    n_sec = 16
    rng = np.random.default_rng(4)
    n = 500
    # Prevailing wind (95%) from due W, clean. Rare wind (5%) from due E, polluted --
    # a genuine, if infrequent, source.
    directions = np.where(rng.random(n) < 0.95, 270 + rng.normal(0, 5, n), 90 + rng.normal(0, 5, n))
    pm = np.where((directions > 250) & (directions < 290), 8 + rng.normal(0, 2, n), 40 + rng.normal(0, 3, n))

    sums = [0.0] * n_sec
    counts = [0] * n_sec
    for d, p in zip(directions, pm):
        sec = int(((d % 360) + 11.25) // 22.5) % n_sec
        sums[sec] += p
        counts[sec] += 1
    mean_pm = [sums[i] / counts[i] if counts[i] else 0.0 for i in range(n_sec)]

    # Reproduces the fixed selection rule in app.main.pollution_rose().
    min_hours = max(3, int(0.02 * n))
    eligible = [i for i in range(n_sec) if counts[i] >= min_hours]
    dominant = labels[max(eligible, key=lambda i: mean_pm[i])]
    old_dominant = labels[int(np.argmax(sums))]

    assert dominant == "E", f"expected the genuinely polluted sector, got {dominant}"
    assert old_dominant == "W", "sanity check: the old sum-based rule should pick the prevailing wind"
    assert dominant != old_dominant
